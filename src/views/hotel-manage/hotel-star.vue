<template>
  <div>
    <div class="filter-container">
      ...
      <el-button v-if="btnAry.findIndex(x => x.id == '537')!=-1" class="filter-item" type="primary" size="mini" icon="el-icon-plus" @click="handleCreate">添加</el-button>
      <el-button v-if="btnAry.findIndex(x => x.id == '539')!=-1" class="filter-item" type="danger" size="mini" icon="el-icon-close" @click="delMore()">删除</el-button>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="星级名称" align="center" >
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" width="250" class-name="fixed-width">
        <template slot-scope="{row,$index}">
          <el-button v-if="btnAry.findIndex(x => x.id == '538')!=-1" type="primary" size="mini" icon="el-icon-edit" @click="handleUpdate(row)">
            编辑
          </el-button>
          <el-button v-if="btnAry.findIndex(x => x.id == '539')!=-1" size="mini" type="danger" icon="el-icon-close" @click="handleDelete(row,$index)">
            删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="editDialogVisible" :title="dialogTitle" class="right-dialog" custom-class="left">
      <el-form ref="editForm" :rules="rules" :model="editForm" label-position="left" label-width="80px" style="width:80%;margin-bottom:50px">
        <el-form-item label="星级名称" prop="rank_name">
          <el-input v-model="editForm.rank_name" placeholder="请输入星级名称" />
        </el-form-item>
...
<script>
import { getHotelStarList,  getHotelStarAdd,  getHotelStarEdit,  getHotelStarDel } from '@/api/hotel-manage/hotel-star'
...
export default {
  ...
  data() {
    return {
      ...
      multipleSelection: [],
      rules: {
        rank_name:[{ required: true, message: '请输入内容', trigger: 'blur' }],
      },
  ...
  mounted () {
    this.getList();
    this.$store.dispatch('user/showBtn', { id: 44 }).then(res => {
      // console.log('res', res)
  ...
  methods: {
    ...
    handleUpdate(item) {
      this.editDialogVisible = true
      this.dialogTitle = item.rank_name+'-编辑'
      this.editId = item.id
      this.editFlag = false
      this.editForm = {
      ...
      this.$refs.editForm.validate((valid) => {
        if (valid) {
          ...
          if (this.editFlag) {
            ...
            getHotelStarAdd(params).then(res => {
              if (res.code === 0) {
                this.$message({
                  type: 'success',
                  message: '添加成功!'
            ...
            getHotelStarEdit(params).then(res => {
              if (res.code === 0) {
                this.$message({
                  type: 'success',
                  message: '编辑成功!'
    ...
    // 点击修改状态
    handleStatus(row) {
      if (this.btnAry.findIndex(x => x.id == '566') == -1) {
        this.$message({
          type: 'warning',
          message: '您没有权限修改'
        })
    ...
    handleDelete(row, index) {
      this.$confirm('确认要删除吗', '提示', {
        ...
        .then(() => {
          ...
          getHotelStarDel(params).then(res => {
            if (res.code === 0) {
              this.$message({
                type: 'success',
                message: '删除成功!'
