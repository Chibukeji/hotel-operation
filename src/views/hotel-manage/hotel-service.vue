<template>
  <div>
    <div class="filter-container">
      ...
      <el-button v-if="btnAry.findIndex(x => x.id == '541')!=-1" class="filter-item" type="primary" size="mini" icon="el-icon-plus" @click="handleCreate">添加</el-button>
      <el-button v-if="btnAry.findIndex(x => x.id == '543')!=-1" class="filter-item" type="danger" size="mini" icon="el-icon-close" @click="delMore()">删除</el-button>
    ...
    <el-tabs v-model="activeName" type="card"  @tab-click="handleClick">
      <el-tab-pane label="酒店设施" name="first"></el-tab-pane>
      <el-tab-pane label="房型设施" name="second"></el-tab-pane>
    </el-tabs>
    ...
    row-key="id" :default-expand-all="expandIf" :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
      ...
      <el-table-column label="操作" align="center" width="250" class-name="fixed-width">
        <template slot-scope="{row,$index}">
          <el-button v-if="btnAry.findIndex(x => x.id == '542')!=-1" type="primary" size="mini" icon="el-icon-edit" @click="handleUpdate(row)">
            编辑
          </el-button>
          <el-button v-if="btnAry.findIndex(x => x.id == '543')!=-1" size="mini" type="danger" icon="el-icon-close" @click="handleDelete(row,$index)">
            删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="editDialogVisible" :title="dialogTitle" class="right-dialog" custom-class="left">
      <el-form ref="editForm" :rules="rules" :model="editForm" label-position="left" label-width="80px" style="width:80%;margin-bottom:50px">
        <el-form-item label="名称" prop="service_name">
          <el-input v-model="editForm.service_name" placeholder="请输入星级名称" />
        </el-form-item>
        <el-form-item label="排序" prop="sorts">
          <el-input v-model="editForm.sorts" placeholder="请输入排序" oninput="value=value.replace(/[^\d]/g,'')"/>
        </el-form-item>
        <el-form-item label="上级" prop="pid">
          <el-select v-model="editForm.pid" placeholder="请选择上级" style="width:100%" :disabled="editFlag == false">
            <el-option label="顶级" :value="0"></el-option>
            <el-option v-for="(item,index) in list" :key="index" :label="item.service_name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
...
<script>
import { getHotelServiceList,  getHotelServiceAdd,  getHotelServiceEdit,  getHotelServiceDel } from '@/api/hotel-manage/hotel-service'
...
export default {
  ...
  data() {
    return {
      ...
      multipleSelection: [],
      rules: {
        service_name:[{ required: true, message: '请输入内容', trigger: 'blur' }],
  ...
  mounted () {
    this.getList1();
    this.$store.dispatch('user/showBtn', { id: 45 }).then(res => {
      // console.log('res', res)
  ...
  methods: {
    ...
    getList1() {
      ...
      getHotelServiceList({is_hotel:this.is_hotel}).then(res => {
    ...
    handleUpdate(item) {
      // console.log(item)
      this.editDialogVisible = true
      this.dialogTitle = item.service_name+'-编辑'
      this.editId = item.id
      this.editFlag = false
      this.editForm = {
    ...
    submitHandle() {
      this.$refs.editForm.validate((valid) => {
        if (valid) {
          ...
          if (this.editFlag) {
            const params = Object.assign(this.editForm,{is_hotel:this.is_hotel}) 
            getHotelServiceAdd(params).then(res => {
              if (res.code === 0) {
                this.$message({
                  type: 'success',
                  message: '添加成功!'
            ...
            let params = {
              id:this.editId,
              service_name:this.editForm.service_name,
              sorts:this.editForm.sorts,
              status:this.editForm.status
            }
            getHotelServiceEdit(params).then(res => {
              if (res.code === 0) {
                this.$message({
                  type: 'success',
                  message: '编辑成功!'
    ...
    // 点击修改状态
    handleStatus(row) {
      if (this.btnAry.findIndex(x => x.id == '567') == -1) {
        this.$message({
          type: 'warning',
          message: '您没有权限修改'
        })
    ...
    handleDelete(row, index) {
      ...
      this.$confirm('确认要删除吗', '提示', {
        ...
        .then(() => {
          ...
          getHotelServiceDel(params).then(res => {
            if (res.code === 0) {
              this.$message({
                type: 'success',
                message: '删除成功!'
