<template>
  <div>
    <el-dialog :visible.sync="detailDialogVisible" :title="dialogTitle" width="80%" class="center-dialog">
            <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
                ...
                <el-table-column label="管理" align="center" width="150" class-name="fixed-width">
                    <template slot-scope="{row}">
                        <el-button v-if="row.level == 7" type="primary" size="mini" @click="countyDetail(row)">查看下级</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
import CountyAgent from './county-agent'
export default {
  components: { Pagination,CountyAgent },
  data() {
    return {
      dialogTitle: "",
  ...
  methods: {
    init(item) {
        ...
        this.dialogTitle = `${item.area_name}-市代理`
